...
int mcsfs1_create(struct mcsfs1_mount *mnt, const char *name) {
    ...
    int rc = valid_name(name, &name_len);
    if (rc != 0) return rc;
    if (find_dirent(mnt->dev, name, 0, 0) == MCSFS1_ERR_OK) return MCSFS1_ERR_EXIST;
    ...
    uint32_t ino = 0;
    uint32_t data_lba = 0;
    rc = alloc_inode_block(mnt->dev, &ino, &data_lba);
    if (rc != 0) return rc;
    ...
    inode.links = 1u;
    inode.size = 0u;
    inode.direct[0] = data_lba;
    rc = write_inode(mnt->dev, ino, &inode);
    if (rc != 0) return rc;
    mcsfs_memset(&de[free_slot], 0, (uint32_t)sizeof(de[free_slot]));
...
int mcsfs1_write(struct mcsfs1_mount *mnt, const char *name, const uint8_t *buf, uint32_t len) {
    if (mnt == 0 || mnt->dev == 0 || buf == 0) return MCSFS1_ERR_INVAL;
    ...
    uint32_t blocks_needed = len == 0u ? 1u : (len + MCSFS1_BLOCK_SIZE - 1u) / MCSFS1_BLOCK_SIZE;
    for (uint32_t i = 0; i < blocks_needed; i++) {
        if (inode.direct[i] == 0u) {
            uint32_t new_lba = 0;
            rc = alloc_data_block(mnt->dev, &new_lba);
            if (rc != 0) return rc;
            inode.direct[i] = new_lba;
        }
    }
    uint8_t block[MCSFS1_BLOCK_SIZE];
    uint32_t written = 0;
...
int mcsfs1_read(struct mcsfs1_mount *mnt, const char *name, uint8_t *buf, uint32_t cap, uint32_t *out_len) {
    ...
    uint8_t block[MCSFS1_BLOCK_SIZE];
    for (uint32_t i = 0; i < blocks_needed; i++) {
        if (inode.direct[i] == 0u || inode.direct[i] >= mnt->block_count) return MCSFS1_ERR_CORRUPT;
