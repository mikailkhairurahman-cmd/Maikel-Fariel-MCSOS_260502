...
static uintptr_t align_down_uintptr(uintptr_t value,
                                    uintptr_t alignment)
{
                         ...
                         uint64_t id)
{
    ...
    uintptr_t top =
        align_down_uintptr(high, MCSOS_STACK_ALIGN);
    ...
    *((uint64_t *)top) = UINT64_C(0);
    ...
    thread->context.rip =
        (uint64_t)(uintptr_t)mcsos_thread_trampoline;
...
int mcsos_sched_yield(mcsos_scheduler_t *sched)
{
    ...
    if (old_thread->state == MCSOS_THREAD_RUNNING &&
        ...
        int rc =
            mcsos_sched_enqueue(sched, old_thread);
    ...
    mcsos_context_switch(&old_thread->context,
                         &next_thread->context);
#endif
