...
{
    return (kmem_block_t *)(
        ((unsigned char *)ptr) - sizeof(kmem_block_t));
}
...
static void kmem_split_if_useful(
    kmem_block_t *block,
    size_t wanted)
{
    const size_t header =
        kmem_align_up_size(sizeof(kmem_block_t), KMEM_ALIGN);
    ...
    unsigned char *new_addr =
        kmem_payload(block) + wanted;

    new_addr = (unsigned char *)
        kmem_align_up_ptr((uintptr_t)new_addr, KMEM_ALIGN);
    ...
    const size_t consumed =
        (size_t)(new_addr - kmem_payload(block));

    if (block->size <=
        consumed + sizeof(kmem_block_t) + KMEM_MIN_SPLIT) {
        return;
    }
    ...
    new_block->size =
        block->size - consumed - sizeof(kmem_block_t);
    new_block->free = 1;
...
int kmem_init(void *base, size_t bytes)
{
    ...
    const size_t lost =
        (size_t)(start - (uintptr_t)base);

    if (bytes <=
        lost + sizeof(kmem_block_t) + KMEM_MIN_SPLIT) {
        return -3;
    }

    size_t usable = bytes - lost;
    usable &= ~(size_t)(KMEM_ALIGN - 1u);

    if (usable <=
        sizeof(kmem_block_t) + KMEM_MIN_SPLIT) {
        return -4;
    }
...
int kmem_validate(void)
{
    ...
    for (kmem_block_t *cur = g_head;
        ...
        if ((unsigned char *)cur < g_heap_base ||
            ((unsigned char *)cur +
             sizeof(kmem_block_t)) > g_heap_end) {
            return -5;
        }
