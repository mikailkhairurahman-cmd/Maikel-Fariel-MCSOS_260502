...
if nm -u build/src/vmm.o | grep -v '^$'; then
  echo "[FAIL] build/vmm.o memiliki unresolved symbol" >&2
  exit 1
fi

objdump -dr build/src/vmm.o > build/vmm.objdump.txt

grep -q "invlpg" build/vmm.objdump.txt || {
  echo "[FAIL] invlpg tidak terlihat pada disassembly" >&2
  exit 1
}

grep -q "cr3" build/vmm.objdump.txt || {
  echo "[FAIL] akses CR3 tidak terlihat pada disassembly" >&2
  exit 1
}
