...
case "$ROOT" in
    /mnt/c/*|/mnt/d/*|/mnt/e/*)
        ...
        ;;
    *)
        log "OK filesystem: repository Linux WSL"
        ;;
esac
...
if [ -f build/meta/toolchain-versions.txt ]; then
    log "OK M1 metadata ditemukan"
else
    log "WARN: metadata M1 belum ada"

    if make -n meta >/dev/null 2>&1; then
        make meta
    else
        fail "target make meta tidak tersedia"
    fi
fi
...
then
    ...
    fail "OVMF tidak ditemukan. Install paket ovmf."
fi
