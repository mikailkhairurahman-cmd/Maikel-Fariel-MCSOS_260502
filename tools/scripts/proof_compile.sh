...
readelf -hW \
"$OUT/freestanding_probe.o" \
| tee "$OUT/readelf-object-header.txt"
...
readelf -SW \
"$OUT/freestanding_probe.elf" \
| tee "$OUT/readelf-sections.txt"

objdump -drwC \
"$OUT/freestanding_probe.o" \
| tee "$OUT/objdump-disassembly.txt"

nm -u \
"$OUT/freestanding_probe.elf" \
| tee "$OUT/nm-undefined.txt"

file \
"$OUT/freestanding_probe.o" \
"$OUT/freestanding_probe.elf" \
| tee "$OUT/file-type.txt"

if [ -s "$OUT/nm-undefined.txt" ]; then
    echo "ERROR: undefined symbols detected in freestanding ELF"
    exit 1
fi
